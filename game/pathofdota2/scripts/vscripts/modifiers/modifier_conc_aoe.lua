modifier_conc_aoe = class({})
modifier_conc_aoe.ability_data = {}
modifier_conc_aoe.kv_values = {}

function modifier_conc_aoe:IsHidden() return false end
function modifier_conc_aoe:IsPermanent() return true end

function modifier_conc_aoe:OnCreated()
...
function modifier_conc_aoe:OnIntervalThink()
    self:OnRefresh()
end

function modifier_conc_aoe:OnRefresh()
    if IsServer() then
        self:OnCreated()
...
function modifier_conc_aoe:AddCustomTransmitterData()
    return {
...
function modifier_conc_aoe:HandleCustomTransmitterData( data )
    self.ability_data = data.ability_data
    self.kv_values = data.kv_values
end

function modifier_conc_aoe:DeclareFunctions()
    return {
...
function modifier_conc_aoe:GetModifierOverrideAbilitySpecial( params )
    if params.ability:IsItem() then return 0 end
    if self.ability_data[ params.ability:GetName() ]==nil then return 0 end
    return 1
end

function modifier_conc_aoe:GetModifierOverrideAbilitySpecialValue( params )
...
function modifier_conc_aoe:CalcKvData()
    if IsServer() then
    -- print("This ran...")
                        ...
                        for l=0,ability:GetMaxLevel()-1 do
                            ...
                            if value then
                                ...
                                if is_aoe and is_ability_damage_KV then
                                    ...
                                    local newDamage = tonumber( findnth(damageStr,l+1) ) * 1.75
