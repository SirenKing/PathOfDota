...
Extend_BaseNPC = CDOTA_BaseNPC or C_DOTA_BaseNPC

function Extend_BaseNPC:PlaySequenceWithRateModifier( act, seq_str, rate, dur )
...
function Extend_BaseNPC:GetDropLuck()
  local luck = 0
...
function Extend_BaseNPC:SetDropLuck( luck )
  if self:HasModifier("modifier_bonus_drop_chance") then
...
function Extend_BaseNPC:MaybeDropItem( killer, luck_bonus )
...
function Extend_BaseNPC:DropItemRewardForHero( killer, rarity_str )
  if BUTTINGS.ENCHANTS_DEBUG then
...
function Extend_BaseNPC:CreateSubcasters( num )
  self.subcasters = {}
  for i=1,num do
...
function Extend_BaseNPC:GetSubcasters()
  return self.subcasters
end

function Extend_BaseNPC:GetIdleSubcasters()
...
function Extend_BaseNPC:DelayCastWithOrders( order_params, delay )
...
function Extend_BaseNPC:SetRandomForward()
  -- self:SetAbsAngles( 0, RandomVector(100).y, 0 ) -- needs to be tested
end

function Extend_BaseNPC:GetDistToPos( pos )
...
function Extend_BaseNPC:GetMainHeroAbilities()
