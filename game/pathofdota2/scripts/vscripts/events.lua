LinkLuaModifier( "modifier_apprentice_follow", "modifiers/modifier_apprentice_follow", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "modifier_apprentice_creep", "modifiers/modifier_apprentice_creep", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "modifier_infernal_legion_burn", "modifiers/modifier_infernal_legion", LUA_MODIFIER_MOTION_NONE )

GEM_EXCEPTIONS = LoadKeyValues('scripts/vscripts/gem_exceptions.txt')
...
ListenToGameEvent("npc_spawned", function(keys)
	...
	if spawnedUnit ~= nil then
		...
		-- APPRENTICE LOGIC
		if string.match(spawnedUnit:GetUnitName(), "npc_dota_creep_") and string.match( spawnedUnit:GetUnitName(), "_ranged") then
			spawnedUnit:AddNewModifier( nil, nil, "modifier_apprentice_creep", {})
		end
		if owner and string.match( spawnedUnit:GetOwner():GetName(), "creep" ) then
			spawnedUnit:AddNewModifier( nil, nil, "modifier_apprentice_follow", {})
		end
		-- INFERNAL LEGION LOGIC
		if owner and string.match( owner:GetName(), "_hero" ) then
			-- print("Owner is a hero")
			if owner:HasModifier( "modifier_infernal_legion" ) then
				-- print("Owner has the modifier")
				local ability_data = owner:FindModifierByName( "modifier_infernal_legion" ).ability_data
				if ability_data then
					DeepPrint(ability_data)
					local should_burn = false
					for k,v in pairs(ability_data) do
						local unit_string_for_ability = GEM_EXCEPTIONS["infernal_legion_unit_string_match"][k]
						-- print(spawnedUnit:GetUnitName(), unit_string_for_ability)
						if string.match( spawnedUnit:GetUnitName(), unit_string_for_ability ) then
							should_burn = true
						end
					end
					if should_burn then
						-- print( "This unit needs to BURN!!!")
						spawnedUnit:AddNewModifier( owner, nil, "modifier_infernal_legion_burn", {})
					end
				end
			end
		end
